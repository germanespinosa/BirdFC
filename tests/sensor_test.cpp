...
TEST_CASE("sensor_data update_value")
{
    ...
    CHECK(sensor_data.value == 20);
    CHECK(sensor_data.change_speed == 2000);
}
...
TEST_CASE("sensor_data accumulative update_value")
{
    ...
    change_speed = 1000;
    sensor_data.update_value(change_speed);
    CHECK(sensor_data.value == 30);
    CHECK(sensor_data.change_speed == 1000);
    change_speed = -5000;
    sensor_data.update_value(change_speed);
    CHECK(sensor_data.value == -20);
    CHECK(sensor_data.change_speed == -5000);
}
